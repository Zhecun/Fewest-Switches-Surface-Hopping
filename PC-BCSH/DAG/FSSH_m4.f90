module constant
    implicit none
    double precision,parameter :: A=6.0D-4
    double precision,parameter :: B=0.10
    double precision,parameter :: C=0.90
    double precision,parameter :: Z=4
...
!********************************************************
    ...
    double precision function V12(x)
        ...
        double precision :: x
        if (x<-Z) then
            V12=-B*exp(C*(x-Z))+B*exp(C*(x+Z))
        elseif (x<Z .and. x>-Z) then
            V12=-B*exp(C*(x-Z))-B*exp(-C*(x+Z))+2*B
        elseif (x>Z) then
            V12=B*exp(-C*(x-Z))-B*exp(-C*(x+Z))
        endif
        return 
...
!********************************************************
    ...
    double precision function D_V12(x)
        ...
        double precision :: x
        if (x<-Z) then
            D_V12=-B*C*exp(C*(x-Z))+B*C*exp(C*(x+Z))
        elseif (x<Z .and. x>-Z) then
            D_V12=-B*C*exp(C*(x-Z))+B*C*exp(-C*(x+Z))
        elseif (x>Z) then
            D_V12=-B*C*exp(-C*(x-Z))+B*C*exp(-C*(x+Z))
